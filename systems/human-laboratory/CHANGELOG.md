...
Measured rather than assumed, and on a page built for the purpose: one card
carrying the shipped class list, printed against each build's own stylesheets and
read back out of the PDF. With the floor its edge draws at `#3A3A3A`, without it
at `#242424`. The components page itself cannot answer this — `--color-ambient`
is legitimately drawn there by the corner brackets and `ImageFrame`'s grid, so
both values appear whether or not the fix is present.
